...
#Con limpieza nos aseguramos que no hay creado ninguna tabla o trigger y si lo hay, lo eliminará
...
#Con create table creamos todas las tablas de la base proyectos informaticos
...
-- tabla copia de eliminados docente
...
#Con Procedimientos DOCENTE nos aseguramos que, al igual que limpieza no exista de antes procedimientos almacenados

-- aqui hay otro drop if exists para hacer limpieza de datos
...
#Con create procedure, creamos los procedimientos almacenados de las tablas
#Algo importante del script son los DELIMITER $$ que le dicen a mysql que no termina el codigo en ; sino en $$
...
#En procedimientos proyectos se hace exactamente lo mismo que en docentes, antes de crear los procedimientos
# se verifica si existen para borarrlos y crearlos en limpio.

#Puse varios delimiter$$ en todo el script para poder hacer pruebas por separado y ver si todo  estaba bien. 
#Además combinar los drops con el resto del codigo me generaba conlfictos
...
/*
 Crea una funcion de promedio, llamada fn_promedio_presupuesto_por_docente, utilizando el id docente
 retorna un numero de 12 enteros y dos decimales
 reads sql consulta los datos de la Bd
 guarda el promedio en v_prom y el promedio lo busca en la tabla proyectos utilizando las variables de docente jefe
 y si el docente no tiene proyecto entonces es null lo que devuelve 0
 */
...
/*
 Con los triggers tenemos una especie de copia de seguridad de los datos, que se guardan en 
 copia_actualizados_docente y copia_eliminados_docente
 en after update, se guarda los cambios despues de que se actualiza docente
 en after delete se guardan los datos que eliminan
 */
-- Triggers
delimiter $$
...
/*
 Con los index creamos indices para que al hacer una busqueda en la tabla, en este caso docente id jefe y numero de documento
 no busque en cada columna, si no vaya directamente a la columna docente id jefe o numero de documento
 */
-- Índices
