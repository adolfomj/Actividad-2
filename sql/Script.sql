...
-- Aqui se cambia el nombre de la columna nombre a proyecto y de nombres de docente a docnete_jefe
...
-- como lo dice el query muestra el promedio del presupueto por docente y cambia el nombre a promedio_presupuesto
...
-- muestra la copia de las actualizaciones de los docente, los muestra en forma descendiente y limita los resultados a 10
-- Q3: Verificar trigger UPDATE (auditoría)
SELECT * FROM copia_actualizados_docente
ORDER BY auditoria_id DESC
LIMIT 10;

-- muesta la copia de los docentes eliminados, en forma descendiente y limita los resultados a 10
-- Q4: Verificar trigger DELETE (auditoría)
SELECT * FROM copia_eliminados_docente
ORDER BY auditoria_id DESC
LIMIT 10;

-- Se hace una consulta de la tabla de proyectos, con condiciones de no tener valores invalidos
-- Q5: Validar CHECKs
...
-- Consulta de los docentes con sus ids y si el docente tiene proyectos aparece el proyecto con id, si no tiene aparece null
...
-- Consulta a los docentes con id y si tiene horas en los proyectos aparece a la derecha, si no aparece null
